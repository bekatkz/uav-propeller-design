...
UPDATED: "Serial Mode" with Progress Bar (Dots) for monitoring speed.
...
def objective_stage1_penalty(x, base_rot1, base_rot2, fl, V_inf, r_R, cfg: Config, T_target: float):
    # --- HEARTBEAT MONITOR ---
    # This prints a dot for every design evaluation so you know it's alive.
...
def main():
    ...
    # 2. Calculate Targets
    W = cfg.mass_kg * cfg.g
    # Each propulsor arm (coaxial pair) lifts 1/8th of the total weight
    T_target_system = W / float(cfg.n_propulsors)
    
    fl = fluid.Fluid(cfg.altitude_m)
    
    # Radius calc: 16 rotors total sharing the load
    ...
    print(f"=== DESIGN CASE: EHang Coaxial (Genetic + SERIAL MODE) ===")
    print(f"Target Thrust: {T_target_system:.2f} N (per arm) | Radius: {R1:.3f} m")
    print(f"Workers: 1 (Serial - No Parallel)")

    sanity_checks(cfg, base_rot1, base_rot2, fl, r_R, x0, max(R1, R2), omega_min, omega_max, T_target_system)

    # --- STAGE 1: GLOBAL EVOLUTIONARY SEARCH (SERIAL) ---
    print("\n=== STAGE 1: Differential Evolution (Global Search) ===")
    t0 = time.time()
    
    # UPDATED: workers=1 for immediate feedback and stability
    res1 = differential_evolution(
        ...
        recombination=0.7,
        disp=True,
        workers=1,          # <--- FIXED: Set to 1 to avoid freezing
        updating='deferred'
    )
    t1 = time.time()

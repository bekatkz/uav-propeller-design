# -*- coding: utf-8 -*-
"""
optimize_required_only_fast_stage2.py

TIME-CRUNCH version of your "minimal required" coaxial prop optimization.

What it does:
- Fixed Ru/Rl = 1 (NOT optimized)
- Optimize: independent chord upper+lower (4 ctrl pts each), independent linear twist upper+lower,
  and tip speed via omega.
- 30 radial stations, root cutout = 0.1R
- Objective: minimum shaft power at design flight state at target thrust (per coax unit)
- Sweep: climb speed Vc +/- 10 m/s in 1 m/s steps, trimming omega to match target thrust at each speed
- Writes at end: results NPZ + summary TXT + YAML upper/lower + PNG plots
- Uses relative paths only

TIME-CRUNCH fixes:
- Stage 2 (COBYLA) uses few iterations + larger step size
- Adds a physical "floor" on power in obj2: P must be >= ~T*V (otherwise non-physical)
...
class Config:
    ...
    # ---- TIME-CRUNCH stage 2 settings ----
    stage2_maxiter: int = 40       # <<< reduce to 20 if you're extremely short on time
    stage2_rhobeg: float = 0.20    # <<< larger steps so it improves faster

    stage1_coupling_iters: int = 5
    stage2_coupling_iters: int = 15   # a bit higher than 10, still not too slow
    ...
    # ---- TIME-CRUNCH physics guard ----
    # Prevent COBYLA from exploiting numerical loopholes that produce "watts" at ~800 N thrust.
    # A physical lower bound is P >= T*V (climb power). We use 0.8*T*V as a robust guard.
    POWER_FLOOR_FACTOR: float = 0.80
...
# =============================================================================
def main():
    # You said you only have a Stage-1 checkpoint: keep this True.
    SKIP_STAGE_1 = True
    ...
    def obj2(x):
        ...
        # --- FIX #1: reject low-thrust (infeasible) points ---
        # This prevents COBYLA from exploring "near-zero thrust => near-zero power" nonsense.
        if (not np.isfinite(P)) or (T < (T_target - 1.0)):   # 1 N margin
            return 1e12
        # -----------------------------------------------------
        
        # (optional) keep the power floor too
        P_floor = cfg.POWER_FLOOR_FACTOR * T * cfg.V_design
        if P < P_floor:
            return 1e12
        
        print(f"COBYLA step: T = {T:.1f} N | P = {P:.1f} W", flush=True)
        return P


    cons = [
        {"type": "ineq", "fun": lambda x: c_thrust_min(x, cache=cache, base_rot1=base_rot1, base_rot2=base_rot2, fl=fl,
                                                      V_inf=cfg.V_design, r_R=r_R, cfg=cfg, T_target=T_target, W=W)},
        {"type": "ineq", "fun": lambda x: c_thrust_max(x, cache=cache, base_rot1=base_rot1, base_rot2=base_rot2, fl=fl,
                                                      V_inf=cfg.V_design, r_R=r_R, cfg=cfg, T_target=T_target, W=W)},
        {"type": "ineq", "fun": lambda x: c_mach(x, base_rot1=base_rot1, base_rot2=base_rot2, fl=fl,
                                                 V_inf=cfg.V_design, r_R=r_R, cfg=cfg, W=W)},
        {"type": "ineq", "fun": lambda x: c_max_coverR(x, base_rot1=base_rot1, base_rot2=base_rot2, fl=fl,
                                                       V_inf=cfg.V_design, r_R=r_R, cfg=cfg, W=W)},
    ...
    for V in Vs:
        omega_trim, res_trim = trim_omega_for_thrust(
            ...
            V_inf=float(V), base_rot1=base_rot1, base_rot2=base_rot2, fl=fl,
            r_R=r_R, cfg=cfg, W=W, T_target=T_target
        )

...
PARALLEL Stage 1 using ThreadPool (robust on Windows; no pickling required)

- Stage 1: Differential Evolution with hinge penalties (T shortfall + Mach violation)
          Uses ThreadPool workers across all CPU cores: workers=pool.map
- Stage 2: COBYLA local refinement with caching (serial; COBYLA isn't parallelized)
- Twist is washout by construction; chord is single-hump then taper by construction.

Design variables x (11):
  0 c_root_nd, 1 c_peak_nd, 2 r_peak, 3 c_tip_nd,
  4 beta1_root, 5 d_beta1, 6 beta2_root, 7 d_beta2,
  8 omega, 9 ratio, 10 s2 (lower chord scale)

IMPORTANT:
- Delete checkpoint_opt0.npy once before first run (old checkpoints incompatible).
"""
...
class Config:
    ...
    W_RATIO_REG: float = 0.0   # set 50..300 if you want ratio closer to 1
    W_S2_REG: float = 80.0     # keeps s2 near 1; tune 20..200
...
def chord_hump_taper(r_R: np.ndarray, c_root_nd: float, c_peak_nd: float, r_peak: float, c_tip_nd: float) -> np.ndarray:
    ...
    cp = float(max(c_peak_nd, max(c0, ct) + 1e-6))
...
def build_rotors_from_x(x, base_rot1, base_rot2, r_R, cfg: Config, W: float):
    ...
    s2 = float(np.clip(float(x[10]), cfg.s2_lb, cfg.s2_ub))
...
def objective_stage1_hinge(x, base_rot1, base_rot2, fl, V_inf, r_R, cfg: Config,
    ...
    penalty_mach = cfg.W_MACH * (max(0.0, -margin) ** 2) * P
...
def plot_results(x_opt, base_rot1, base_rot2, fl, r_R, cfg: Config, T_target: float, W: float):
    ...
    speeds = np.arange(0.0, cfg.V_design + 10.0 + 0.1, 1.0)
...
def main():
    SKIP_STAGE_1 = False  # run DE once for this new 11-var formulation
    ...
    # Mach-safe omega_max for worst-case Rmax at ratio_ub
    R1_w, R2_w, A_total = compute_radius_pair_from_ratio(W, cfg.disk_loading, cfg.n_propulsors, cfg.ratio_ub)
    Rmax_w = max(R1_w, R2_w)
    omega_max = cfg.M_tip_max * float(fl.a) / float(Rmax_w)

    print(f"Target_Area(total coaxial): {A_total:.4f} m^2")
    print(f"Worst-case Rmax (ratio={cfg.ratio_ub:.2f}): {Rmax_w:.3f} m")
    ...
    # ----------------------------
    # STAGE 1: Differential Evolution (ThreadPool parallel)
    # ----------------------------
    if SKIP_STAGE_1 and os.path.exists(checkpoint_file):
        ...
        try:
            res1 = differential_evolution(
                ...
                tol=0.01,
                workers=pool.map,          # <-- all cores via threads (no pickling)
                updating="deferred",
                callback=cb_save,
                disp=True,
                polish=True,
...
if __name__ == "__main__":
    # Good practice on Windows (safe even if not strictly needed with ThreadPool)
    mp.freeze_support()
    main()
